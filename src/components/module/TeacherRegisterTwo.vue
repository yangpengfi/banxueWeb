<template>
    <div>
        <div class="register-content">
            ...
            <p>
                <span><i style="color:red;">*</i> 教龄：</span>
                <input type="number" v-model="teachAge" placeholder="请输入教龄" min="0" max="60" oninput="if(value.length>2)value=value.slice(0,2);if(value>60)value=60;if(!/^[0-9]\d*$/.test(value))value=''">
            </p>
            <p class="level">
                <span>学历：</span>
                <input type="text" placeholder="请输入学历" v-model="education" maxlength="10" title="长度不超过15位">
            </p>
            <p class="level">
                <span>毕业院校：</span>
                <input type="text" placeholder="请输入毕业院校" v-model="school" maxlength="25" title="长度不超过25位">
            </p>
            <p>
                <span>民族：</span>
            ...
            <button type="button" id="next-btn" @click="registerTeacher">确认注册</button>
        </div>
    </div>
</template>
<script>
export default {
    name:'TeacherRegisterOne',
    data(){
        return {
           trueName:'',
           teachAge:'',
